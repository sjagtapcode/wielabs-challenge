...
export const getDataFromCard = (card: Element, tag: string) => {
  const elements = Array.from(card?.querySelectorAll('span'))?.filter((ele) => ele?.textContent?.includes(tag))
...
export const getLinksfromCard = (card: Element) : {
  linkedInUrl: string,
  twitterUrl: string,
} => {
  const linkedInUrl = card?.querySelector('a[title="LinkedIn profile"]')?.getAttribute('href') || '';
  const twitterUrl = card?.querySelector('a[title="Twitter account"]')?.getAttribute('href') || '';
  return {
    linkedInUrl,
    twitterUrl
  }
...
export const getFoundersList = (document: Document) => {
  ...
  const foundersHeader = h3Headers?.find((header) => header.textContent === 'Active Founders');
  const foundersListNodes = Array.from(foundersHeader?.parentElement?.nextSibling?.childNodes || []);
  const founderList = foundersListNodes?.map((founder) => {
    ...
    const imageUrl = document.querySelector(`img[alt="${name}"]`)?.getAttribute('src') || '';
    const socialUrlNodes = Array.from(founderCard?.[1]?.childNodes?.[2]?.childNodes);
    const links = {
      linkedInUrl: '',
      twitterUrl: '',
    }
