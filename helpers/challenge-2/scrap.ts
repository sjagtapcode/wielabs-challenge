...
export const webScrap = async (websiteUrls: string[]): Promise<IWebsiteData[]> => {
  await crawler.run(websiteUrls);
  await jobsCrawler.run([]);
  await launchCrawler.run([]);
  return await Promise.all(websiteUrls?.map(async (websiteUrl) => {
    ...
    if (allData?.launches?.length) {
      const launchLinks = allData?.launches?.map(({ link }: ILaunchSummary) => link)
